...
package com.embeded.alarmclock;
...
import com.embeded.deskclock.DeskClock;
import com.embeded.deskclock.Utils;
import com.embeded.deskclock.alarms.AlarmStateManager;
import com.embeded.deskclock.worldclock.Cities;
import com.embeded.deskclock.worldclock.CitiesActivity;
...
public class DigitalAppWidgetProvider extends AppWidgetProvider {
                ...
                for (int appWidgetId : appWidgetIds) {
                    appWidgetManager.
                            notifyAppWidgetViewDataChanged(appWidgetId,
                                    com.embeded.deskclock.R.id.digital_appwidget_listview);
                    RemoteViews widget = new RemoteViews(context.getPackageName(),
                            com.embeded.deskclock.R.layout.digital_appwidget);
                    float ratio = WidgetUtils.getScaleRatio(context, null, appWidgetId);
                    WidgetUtils.setTimeFormat(context, widget, 0/*no am/pm*/, com.embeded.deskclock.R.id.the_clock);
    ...
    private void updateClock(
        ...
        RemoteViews widget = new RemoteViews(context.getPackageName(), com.embeded.deskclock.R.layout.digital_appwidget);
        ...
        if (newOptions != null &&
            ...
            widget.setOnClickPendingIntent(com.embeded.deskclock.R.id.digital_appwidget,
        ...
        final CharSequence dateFormat = Utils.isJBMR2OrLater()
                ? DateFormat.getBestDateTimePattern(Locale.getDefault(),
                        context.getString(com.embeded.deskclock.R.string.abbrev_wday_month_day_no_year))
                : ((SimpleDateFormat) SimpleDateFormat.getDateInstance(SimpleDateFormat.SHORT))
                        .toPattern();
        widget.setCharSequence(com.embeded.deskclock.R.id.date, "setFormat12Hour", dateFormat);
        widget.setCharSequence(com.embeded.deskclock.R.id.date, "setFormat24Hour", dateFormat);
        ...
        widget.setRemoteAdapter(com.embeded.deskclock.R.id.digital_appwidget_listview, intent);

        // Set up the click on any world clock to start the Cities Activity
        //TODO: Should this be in the options guard above?
        widget.setPendingIntentTemplate(com.embeded.deskclock.R.id.digital_appwidget_listview,
                PendingIntent.
                        getActivity(context, 0, new Intent(context, CitiesActivity.class), 0));

        // Refresh the widget
        appWidgetManager.notifyAppWidgetViewDataChanged(
                appWidgetId, com.embeded.deskclock.R.id.digital_appwidget_listview);
    ...
    protected void refreshAlarm(Context context, RemoteViews widget) {
        ...
        if (!TextUtils.isEmpty(nextAlarm)) {
            widget.setTextViewText(com.embeded.deskclock.R.id.nextAlarm,
                    context.getString(com.embeded.deskclock.R.string.control_set_alarm_with_existing, nextAlarm));
            widget.setViewVisibility(com.embeded.deskclock.R.id.nextAlarm, View.VISIBLE);
            if (DigitalAppWidgetService.LOGGING) {
                Log.v(TAG, "DigitalWidget sets next alarm string to " + nextAlarm);
            }
        } else  {
            widget.setViewVisibility(com.embeded.deskclock.R.id.nextAlarm, View.GONE);
            if (DigitalAppWidgetService.LOGGING) {
