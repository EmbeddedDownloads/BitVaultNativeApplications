...
import com.embeded.deskclock.worldclock.CityObj;
import com.embeded.deskclock.worldclock.WorldClockAdapter;
...
public class DigitalWidgetViewsFactory implements RemoteViewsFactory {
    ...
    private class RemoteWorldClockAdapter extends WorldClockAdapter {
        ...
        public RemoteWorldClockAdapter(Context context) {
            ...
            mClocksPerRow = context.getResources().getInteger(
                    com.embeded.deskclock.R.integer.appwidget_world_clocks_per_row);
            mFontSize = context.getResources().getDimension(com.embeded.deskclock.R.dimen.widget_medium_font_size);
            mFont24Size = context.getResources().getDimension(com.embeded.deskclock.R.dimen.widget_24_medium_font_size);
        }
        ...
        public RemoteViews getViewAt(int position) {
            ...
            RemoteViews views = new RemoteViews(
                    mContext.getPackageName(), com.embeded.deskclock.R.layout.world_clock_remote_list_item);

            // Always how the left clock
            updateView(views, (CityObj) mCitiesList[index], com.embeded.deskclock.R.id.left_clock,
                    com.embeded.deskclock.R.id.city_name_left, com.embeded.deskclock.R.id.city_day_left);
            ...
            if (index + 1 < mCitiesList.length) {
                updateView(views, (CityObj) mCitiesList[index + 1], com.embeded.deskclock.R.id.right_clock,
                        com.embeded.deskclock.R.id.city_name_right, com.embeded.deskclock.R.id.city_day_right);
            } else {
                hideView(views, com.embeded.deskclock.R.id.right_clock, com.embeded.deskclock.R.id.city_name_right,
                        com.embeded.deskclock.R.id.city_day_right);
            }

            // Hide last spacer if last row
            int lastRow = ((mCitiesList.length + 1) / 2) - 1;
            if (position == lastRow) {
                views.setViewVisibility(com.embeded.deskclock.R.id.city_spacer, View.GONE);
            } else {
                views.setViewVisibility(com.embeded.deskclock.R.id.city_spacer, View.VISIBLE);
            }
        ...
        private void updateView(RemoteViews clock, CityObj cityObj, int clockId,
            ...
            final int labelSize = mResources.getDimensionPixelSize(com.embeded.deskclock.R.dimen.widget_label_font_size);
            ...
            if (myDayOfWeek != cityDayOfWeek) {
                clock.setTextViewText(dayId, mContext.getString(
                        com.embeded.deskclock.R.string.world_day_of_week_label, now.getDisplayName(
    ...
    public RemoteViews getViewAt(int position) {
        ...
        if (v != null) {
            ...
            v.setOnClickFillInIntent(com.embeded.deskclock.R.id.widget_item, fillInIntent);
        }
        return v;
    }
