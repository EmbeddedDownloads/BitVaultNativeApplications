...
public class WidgetUtils {
    ...
    public static void setClockSize(Context context, RemoteViews clock, float scale) {
        float fontSize = context.getResources().getDimension(com.embeded.deskclock.R.dimen.widget_big_font_size);
        clock.setTextViewTextSize(
                com.embeded.deskclock.R.id.the_clock, TypedValue.COMPLEX_UNIT_PX, fontSize * scale);
    }
    ...
    public static float getScaleRatio(Context context, Bundle options, int id) {
        ...
        if (options != null) {
            ...
            float ratio = (density * minWidth) / res.getDimension(com.embeded.deskclock.R.dimen.min_digital_widget_width);
            // Check if the height could introduce a font size constraint
            int minHeight = options.getInt(AppWidgetManager.OPTION_APPWIDGET_MIN_HEIGHT);
            if (minHeight > 0 && (density * minHeight)
                    < res.getDimension(com.embeded.deskclock.R.dimen.min_digital_widget_height)) {
    ...
    private static float getHeightScaleRatio(Context context, Bundle options, int id) {
        ...
        if (options != null) {
            ...
            float lblBox = 1.35f * res.getDimension(com.embeded.deskclock.R.dimen.label_font_size);
            // Ensure divisor for ratio is positive number
            if (res.getDimension(com.embeded.deskclock.R.dimen.min_digital_widget_height) - lblBox > 0) {
                float ratio = ((density * minHeight) - lblBox)
                        / (res.getDimension(com.embeded.deskclock.R.dimen.min_digital_widget_height) - lblBox);
                return (ratio > 1) ? 1 : ratio;
            }
        }
        return 1;
    ...
    public static boolean showList(Context context, int id, float scale) {
        ...
        float lblBox = 1.35f * res.getDimension(com.embeded.deskclock.R.dimen.label_font_size);
        float neededSize = res.getDimension(com.embeded.deskclock.R.dimen.digital_widget_list_min_fixed_height) +
                2 * lblBox +
                scale * res.getDimension(com.embeded.deskclock.R.dimen.digital_widget_list_min_scaled_height);
